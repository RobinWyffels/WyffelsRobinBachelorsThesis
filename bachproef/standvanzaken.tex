...
\begin{itemize}
\item \textbf{Links:} Dit zijn de rigide onderdelen van de robot, zoals het chassis, de wielen of sensorbehuizingen. Per link worden drie cruciale eigenschappen gedefinieerd: de \textit{visual} (hoe het onderdeel eruitziet in een graphical user interface), de \textit{collision} (de geometrische vorm die wordt gebruikt voor botsingsdetectie in simulatoren) en de \textit{inertial} (de massa en traagheidsmomenten). Vooral de \textit{inertial}-tags zijn van vitaal belang voor physics engines zoals die in NVIDIA Isaac Sim om realistische bewegingen te berekenen \autocite{Joseph2018}.
...
In de moderne robotica-ontwikkeling begint de creatie van een robot vrijwel altijd in een \textit{Computer-Aided Design} (CAD) omgeving. Voor dit onderzoek is gebruikgemaakt van Onshape, een cloud-native CAD-platform ontwikkeld door Parametric Technology Corporation (PTC). Hoewel CAD-software essentieel is voor het mechanisch ontwerp, is de resulterende geometrie niet direct bruikbaar voor ROS 2 of fysica-simulatoren zonder een vertaalslag naar een kinematisch formaat zoals URDF.
...
\item \textbf{Mesh-optimalisatie:} De complexe CAD-geometrie wordt vertaald naar een \textit{mesh}. Een mesh is een verzameling van polygonen (meestal driehoeken) die samen het oppervlak van een driedimensionaal object vormen. Bij de export naar formaten zoals STL of OBJ is optimalisatie noodzakelijk door een onderscheid te maken tussen:
    \begin{itemize}
        \item \textbf{Visual mesh:} Een grafisch gedetailleerde weergave die zorgt voor een realistische rendering van de robot in de simulator.
        \item \textbf{Collision mesh:} Een sterk vereenvoudigde geometrische vorm (vaak opgebouwd uit eenvoudige cilinders of boxen) die enkel dient voor de botsingsdetectie. Door deze vereenvoudiging wordt de rekenlast voor de \textit{physics engine} aanzienlijk beperkt, wat cruciaal is voor de stabiliteit en real-time prestaties van de simulatie.
    \end{itemize}
\end{enumerate}
...
Een van de meest kritieke uitdagingen bij de ontwikkeling van autonome robots in virtuele omgevingen is de \textit{sim-to-real gap}. Dit fenomeen beschrijft de discrepantie tussen de prestaties van een algoritme in een simulator en de prestaties op de fysieke hardware \autocite{Hoefer2021}. Hoewel deze kloof vaak wordt geassocieerd met visuele verschillen of onnauwkeurige fysische modellering van frictie en lichtinval \autocite{Tan2018, Hoefer2021}, toont recent onderzoek aan dat de oorzaak dieper ligt in de technische integratie tussen de simulator en de robotica-middleware \autocite{Hoefer2021, Peng2020}.
