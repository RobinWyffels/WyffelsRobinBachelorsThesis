...
De automatisering van magazijnen heeft de afgelopen jaren een steeds belangrijkere rol gekregen in sectoren waar efficiëntie, kostenbesparing en veiligheid centraal staan. Autonome robots werden traditioneel ontwikkeld en getest in echte omgevingen, wat tijdrovend en duur is. Een belangrijke vraag is daarom: Hoe goed presteert een robot die in een virtuele omgeving is getraind in de echte wereld? Dit vormt de centrale focus van deze bachelorproef.

De casus richt zich op het ontwerpen, bouwen en programmeren van een autonome robot die dozen kan verplaatsen in een magazijn. De robot zal volledig vanaf nul worden ontwikkeld, inclusief hardware (chassis, aandrijving, hefmechanisme) en software (Ros~2 nodes voor interne systeemcommunicatie, externe communicatie en besturingssystemen). Het innovatieve concept is het gebruik van NVIDIA Isaac Sim en Isaac Lab om de modellen te trainen en de robot virtueel te simuleren. Deze bachelorproef richt zich op IT\-professionals en ingenieurs in industriële automatisering, evenals bedrijven die geïnteresseerd zijn in het optimaliseren van magazijnautomatiseringstechnologieën.

Om de centrale onderzoeksvraag te kunnen beantwoorden, worden eerst enkele deelvragen onderzocht die betrekking hebben op het probleemdomein. Deze vragen brengen de aard van het probleem, de moeilijkheden van magazijnomgevingen en de technische uitdagingen van sim\-to\-real overdracht in kaart:

\begin{itemize}
    \item Welke factoren veroorzaken typisch een sim\-to\-real kloof bij autonome mobiele robots in magazijnen?
    \item Welke beperkingen hebben sensoren zoals Lidar en camera’s in realistische magazijnomstandigheden?
    \item Hoe complex en variabel zijn echte magazijnomgevingen in vergelijking met gesimuleerde omgevingen?
    \item Welke uitdagingen bestaan er in Ros~2 en de Nav2\-navigatiestack bij gebruik in dynamische magazijnomgevingen?
    \item Waarom is het moeilijk om een virtuele omgeving te creëren die realistische fysica, frictie en sensordata correct nabootst?
\end{itemize}

Deze probleemdomeinvragen schetsen het kader waarbinnen sim\-to\-real overdracht moet worden begrepen en verklaren waarom dit een relevant en niet\-triviaal onderzoeksdomein is.

Vervolgens worden deelvragen geformuleerd die betrekking hebben op het oplossingsdomein, namelijk de prestaties van de virtueel getrainde robot in de echte wereld:

\begin{itemize}
    \item Hoe betrouwbaar zijn mapping, navigatie en obstakelvermijding van de robot in een echte magazijnomgeving?
    \item Hoe goed komt het gedrag van de robot in de echte wereld overeen met het gedrag in de virtuele trainingsomgeving?
    \item Wat is de invloed van omgevingsfactoren (licht, vloerfrictie, onverwachte obstakels) op de prestaties van de robot?
    \item Hoe kan het AI\-model worden aangepast op basis van real\-world feedback om prestaties te verbeteren?
    \item Welke parameters in de virtuele trainingsomgeving hebben de grootste invloed op sim\-to\-real prestaties?
\end{itemize}

De centrale onderzoeksvraag van deze bachelorproef luidt:
Hoe vertaalt het gedrag en de prestaties van een virtueel getrainde autonome robot zich naar een echte magazijnomgeving?

Het doel van dit onderzoek is het realiseren van een proof\-of\-concept: een werkende robot die autonoom taken kan uitvoeren in een magazijn, waarbij het gedrag en de prestaties worden vergeleken met de gesimuleerde omgeving. Het eindresultaat omvat een functioneel prototype, een analyse van de verschillen tussen gesimuleerde en reële prestaties, en de bevindingen van het gebruik van NVIDIA Isaac Sim en Isaac Lab voor virtuele training van autonome industriële robots.
%---------- Stand van zaken ---------------------------------------------------
...
\autocite{NVIDIA2025, IsaacLab2025, Makoviychuk2021}
...
De publicatie in \textit{Science Robotics} door 
\textcite{Macenski2022}
beschrijft het ontwerp, de architectuur en toepassingen “in het wild,” wat Ros~2 legitimeert voor veiligheidkritische omgevingen zoals magazijnen. 
...
\subsubsection{Probleemdomein}
\begin{itemize}
    \item \textbf{Analyse van sim-to-real factoren:}
    \begin{itemize}
        \item Literatuurstudie van AMR-onderzoeken sim-to-real overdracht.
        \item Inventarisatie van bekende sim-to-real failure cases.
        \item Analyse van sensor- en fysicamodellen in Isaac Sim.
    \end{itemize}

    \item \textbf{Sensorbeperkingen in real‑world omstandigheden:}
    \begin{itemize}
        \item Testen van Lidar en camera in verschillende lichtcondities.
        \item Meten van sensorruis in statische en dynamische situaties.
        \item Vergelijking van raw sensor streams (sim vs real).
    \end{itemize}

    \item \textbf{Complexiteit van magazijnomgevingen:}
    \begin{itemize}
        \item Observatie en documentatie van magazijnlay-outs.
        \item Analyse van variabiliteit (bewegende objecten, reflecties, menselijke interactie).
        \item Opstellen van een “realism checklist” voor simulatie.
    \end{itemize}

    \item \textbf{Uitdagingen in ROS~2 / Nav2:}
    \begin{itemize}
        \item Analyse van Nav2-logs tijdens real‑world navigatie.
        \item Testen van recovery behaviors (spin, clear costmap).
        \item Vergelijking van planner performance in sim vs real.
    \end{itemize}

    \item \textbf{Beperkingen van simulatie-fysica:}
    \begin{itemize}
        \item Evaluatie van Isaac Sim fysica-instellingen (frictie, inertie, botsingen).
        \item Testen van domain randomization.
        \item Vergelijking van gesimuleerde en gemeten robotdynamica.
    \end{itemize}
\end{itemize}

\subsubsection{Oplossingsdomein}
\begin{itemize}
    ...
    \item \textbf{AI-model bijsturen:} Test effect van modelaanpassingen op taakvoltooiing.
\end{itemize}
